...
urlpatterns = [
    ...
    # 校验用户名
    url(r'^accounts/(?P<username>[a-zA-Z0-9_-]{5,20})/sms/token/$', views.UsernameVerifyView.as_view()),

    # 发送短信
    url(r'^sms_codes/$', views.SendSMSView.as_view()),

    # 校验手机号
    url(r'^accounts/(?P<username>[a-zA-Z0-9_-]{5,20})/password/token/$', views.MobileVerifyView.as_view()),
