...
class UsernameVerifyView(View):
    def get(self, request, username):
        image_code = request.GET.get('text')
        image_code_id = request.GET.get('image_code_id')
        if all([image_code_id,  image_code]) is False:
            return http.HttpResponseForbidden('参数不全')

        redis_conn = get_redis_connection('verify_code')
        redis_image_code = redis_conn.get('img_%s' % image_code_id)
        if redis_image_code is None or redis_image_code.lower().decode() != image_code.lower():
        ...
        mobile_code = '%s' % mobile[0:3] + '****' + '%s' % mobile[7:11]
        data = {'access_token': mobile}
        serializer = Serializer(secret_key=settings.SECRET_KEY, expires_in=3600 * 24)
        access_token = serializer.dumps(data).decode()
        return http.JsonResponse({'access_token': access_token, 'mobile': mobile_code})
...
class MobileVerifyView(View):
    def get(self, request, username):
        ...
        mobile_redis = conn_redis.get('sms_%s' % mobile)
        if mobile_redis is None or sms_code != mobile_redis.decode():
...
class NewPassword(View):
    def post(self, request, user_id):
        ...
        if all([access_token, password, password]) is False:
            return http.HttpResponseForbidden('缺少必传参数')
        if password != password2:
            return http.JsonResponse({'status': 400})
        serializer = Serializer(secret_key=settings.SECRET_KEY, expires_in=3600 * 24)
        try:
            ...
            if int(user.mobile) != int(access_token):
                return http.JsonResponse({'message': '密码修改错误'})
            else:
                user.set_password(password)
                user.save()
                return http.JsonResponse({'redirec': 'true'})
